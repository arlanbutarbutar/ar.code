                                        ...
                                        <?php }else if(mysqli_num_rows($news_reviews)>0){while($row=mysqli_fetch_assoc($news_reviews)){?>
                                            <div class="card border-0 shadow mb-3">
                                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                                    <h6 class="m-0 font-weight-bold" <?= $color_black ?>><?= $row['']?></h6>
                                                    <div class="dropdown no-arrow">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink">
                                                            ...
                                                            <a class="dropdown-item bg-transparent card-scale" <?= $color_black ?> href="notes?id=<?= $row['id_data']?>">Repair View</a>
                                                            <a class="dropdown-item bg-transparent card-scale" <?= $color_black ?> href="qr?id=<?= $row['id_data']?>">View Barcode</a>
                                                            <a class="dropdown-item bg-transparent card-scale" <?= $color_black ?> href="notes-edit?id=<?= $row['id_data']?>">Repair Edit</a>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="card-body">
                                                    <p <?= $color_black ?>>
                                                        Perbaikan <?= $row['product']?> dengan kerusakan <?= $row['kerusakan']?> dan kondisi <?= $row['kondisi']?>. Kelengkapan dari <?= $row['product']?> <?php if(empty($row['kelengkapan']) || $row['kelengkapan']=='-'){echo 'tidak ada.';}else{echo $row['kelengkapan'];}?>. Perbaikan dikerjakan oleh <?php $id_tek=$row['first_name'];$teknisi=mysqli_query($conn, "SELECT * FROM users WHERE id_user='$id_tek'");$row_tek=mysqli_fetch_assoc($teknisi);echo $row_tek['first_name'];?>
                                                    </p>
                                                    <div class="row no-gutters align-items-center">
                                                        <div class="col-auto">
                                                        ...
                                                        <div class="col">
                                                            <div class="progress progress-sm mr-2">
                                                                <div class="progress-bar progress-bar-striped bg-danger" role="progressbar" style="width: <?= $row['progress']."%"?>" aria-valuenow="<?= $row['progress']?>" aria-valuemin="0" aria-valuemax="100"></div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
