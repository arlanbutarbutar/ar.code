<?php if(!isset($_SESSION)){session_start();}
    ...
    $_SESSION['page-name']="Sparepart Dipakai";
?>

<!-- == Sparepart Dipakai page == -->
<!DOCTYPE html>
<html lang="id">
    <head>
                                    ...
                                    <div class="col-lg-12">
                                        <div class="card card-body border-0 shadow mt-3" style="overflow-x: auto">
                                            <table class="table table-sm text-center" <?= $color_black?>>
                                                ...
                                                <tbody>
                                                    <?php $no=1;if(mysqli_num_rows($report_spareparts_take)==0){?>
                                                    <tr>
                                                        <th colspan="10">Belum ada data yang dimasukan hari ini!</th>
                                                    </tr>
                                                    <?php }else if(mysqli_num_rows($report_spareparts_take)>0){while($row=mysqli_fetch_assoc($report_spareparts_take)){?>
                                                    <tr>
                                                        <th scope="row"><?= $no;?></th>
                                                        <td><?= $row['tgl_masuk']?></td>
                                                        <?php if($_SESSION['id-role']<=2){?>
                                            ...
                                            <nav class="small" aria-label="Page navigation example">
                                                <ul class="pagination justify-content-center">
                                                    ...
                                                    <li class="page-item shadow">
                                                        <a class="page-link border-0" <?= $bg_black?> href="report-spareparts-pickup?page=<?= $page16-1;?>" tabindex="-1" aria-disabled="true">Previous</a>
                                                    </li>
                                                    <?php endif;?>
                                                    <?php for($i=1; $i<=$total_page16; $i++):?>
                                                        <?php if($i<=5):?>
                                                            <?php if($i==$page16):?>
                                                                <li class="page-item shadow"><a class="page-link font-weight-bold border-0" <?= $bg_black?> href="report-spareparts-pickup?page=<?= $i;?>"><?= $i;?></a></li>
                                                            <?php else :?>
                                                                <li class="page-item shadow"><a class="page-link border-0" href="report-spareparts-pickup?page=<?= $i;?>"><?= $i;?></a></li>
                                                            <?php endif;?>
                                                        <?php endif;?>
                                                    <?php endfor;?>
                                                    <?php if($page16<$total_page16):?>
                                                    <li class="page-item shadow">
                                                        <a class="page-link border-0" <?= $bg_black?> href="report-spareparts-pickup?page=<?= $page16+1;?>">Next</a>
                                                    </li>
                                                    <?php endif;}}?>
                                                </ul>
                                            </nav>
